    ...
    if($action == 'register')
    {
        ...
        {   
            ...
            $mensagem = "Preencha o".($plural ? 's ': ' ')."campo".($plural ? 's ': ' ').implode(' e ', $campos); 
             $ret_msg = "<p class='text-danger text-center'>$mensagem</p>";
        }        
        else
        {
            $ret = createUser($conn, $nome, $email, $senha);
        
            if($ret['status'] == 'success')
            {
                $ret_msg = "<p class='text-$ret[tag] text-center'>$ret[msg]</p>";
            }
            else
               $ret_msg = "<p class='text-$ret[tag] text-center'>$ret[msg]</p>";
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="criar_conta.php" method="POST">
                            ...
                            <div id="msg_box"></div>                       
                            <button type="submit" name="action" value="register" class="btn btn-lg btn-info btn-block">Cadastrar Usuário</button>
                        </form>
